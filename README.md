...
Problem Statement 4 — Litmus Internship / Hackathon Submission
...
| **RAG / Search Layer** | Lightweight skill-overlap retrieval; Gemini prompts are grounded strictly in the loaded `ROLES` dataset — it's instructed not to invent roles or skills outside it |
| **Intelligence Layer** | `call_gemini()` — role matching, skill-gap explanation, and roadmap generation, with a deterministic rule-based fallback if no key is present or a call fails |
| **Application Layer** | Streamlit, 5 tabs: Profile & Skills · Role Matcher · Skill-Gap & Roadmap · Indic Assistant · Responsible AI & Feedback, plus a separate Mentor Dashboard persona |
...
- **Resume upload (PDF/DOCX)** — auto-fills branch, skills, and a projects summary via best-effort keyword extraction; always presented for review before saving, never auto-submitted
...
- **Gemini-powered reasoning** — via the current `google-genai` SDK, with automatic fallback across multiple model IDs if one gets deprecated (Google has retired several Gemini model IDs through 2026)
- **Sarvam AI Indic integration**:
  - **Translate** — answers translated into 11 Indian languages + Hinglish
  - **Bulbul (TTS)** — 29 selectable voices, standalone tool to read roadmaps/answers/custom text aloud, with downloadable audio
...
- **Mentor Dashboard** — persona switch showing role-family distribution and aggregate skill gaps across the student profile dataset
...
| Indic AI | Sarvam AI (Chat, Translate, Bulbul TTS, Saaras STT) — direct REST calls |
...
```
- Gemini key: [aistudio.google.com/apikey](https://aistudio.google.com/apikey) (free tier available)
...
- Resume parsing is **best-effort keyword matching**, not true NLP extraction — always surfaced for user review before being saved as a profile, never auto-submitted.
- Skill-gap radar chart values are illustrative (randomized within a plausible range) rather than a precisely measured proficiency score.
- Sarvam API calls depend on network reachability to `api.sarvam.ai`; the app retries on timeout and fails gracefully with a clear message rather than crashing.
- Gemini model availability shifts as Google retires older model IDs — `call_gemini()` tries a short chain of current candidates automatically, with a manual override available if needed.
...
- Retrieval-augmented generation over a larger, embeddings-based knowledge base instead of direct JSON context injection
- Expanded resume parsing using a dedicated NLP/NER pipeline for higher-confidence extraction
